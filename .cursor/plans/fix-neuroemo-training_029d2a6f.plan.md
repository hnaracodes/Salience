...
overview: Harden the NeuroEmo ROI training path around semantic vertex alignment, surface-native Schaefer labels, explicit temporal-contract enforcement, atlas metadata correctness, and stronger linear-model baselines while preserving the current 10-TR contiguous, neutral-excluded experiment setup.
todos:
  ...
  - id: temporal-contract
    content: Make dataset prep/trainer temporal settings explicit around the current TR-level NPZ plus 10-TR training-window contract, and guard against future silent double-windowing.
    status: pending
  - id: atlas-metadata-cleanup
    content: Fix atlas label/network parsing and remove unstable legacy parcellation fallbacks.
    status: pending
  - id: linear-baselines
    content: Add LinearSVC-style baselines and standardize fair comparisons against the current 10-TR, mean/std/mean_abs, neutral-excluded experiments.
    status: pending
isProject: false
---
...
- Stop synthesizing temporal context when temporal aggregation is requested. If the trainer is asked to aggregate but true `t_idx` / `time_s` metadata are missing, fail loudly.
...
- Make the default comparison matrix mirror the current serious experiments: `neutral` excluded, `temporal_window_trs=10`, `temporal_contiguity=contiguous`, `roi_reducers=mean,std,mean_abs`, and subject-held-out evaluation only.
- Standardize experiment outputs in the metrics JSON: balanced accuracy, macro F1, per-class F1, confusion matrix, fold variance, temporal settings, atlas id, ROI reducer set, and whether calibration was used.
...
- Add focused tests around:
  ...
  - enforcement of the current TR-level prep plus 10-TR training-window contract,
  - rejection of accidental double temporal aggregation,
...
- Run controlled experiment sets where only one axis changes at a time:
  1. current atlas/current linear baseline under the current 10-TR contiguous settings,
  2. fixed atlas/current linear baseline under the same settings,
  3. fixed atlas + `linear_svc`,
  4. fixed atlas + calibrated specialist comparison,
