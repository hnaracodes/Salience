# Salience — Phased delivery & checkpoints

This document breaks [`salience-architecture-plan.md`](salience-architecture-plan.md) into **concrete phases** with **exit criteria** you can verify independently. The overall vision is large (research-grade neuro-modeling + realtime 3D + agentic UX); these phases are deliberately **narrow** so each gate delivers a demo-able artifact before scope expands.
...
The checklist at the top of [`salience-architecture-plan.md`](salience-architecture-plan.md) maps roughly as:
